...
- **Standalone-game-page fallback (#92) — DEFERRED with rationale**: on a single-file app a "new tab" still parses the whole ~3 MB bundle (no parse-time win; only a fresh heap), and a real solo-boot path would touch the shared boot sequence for 350 users. Left for a user architecture call rather than risk boot. Delivered instead: the games *reorg* (committed) plus a separate batch adding new games + a Crossy Road upgrade.
