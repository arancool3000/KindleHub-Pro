...
- Commit when the user asks; **current dev branch is `claude/claude-md-review-fguoni`** (older round-notes below reference the prior `claude/keen-tesla-n73rpc`). The user merges PRs (or asks me to).
  End commit messages with the session URL line.

## ⚡⚡ THIS SESSION (branch `claude/claude-md-review-fguoni`) — READ FIRST
Large autonomous batch, each item its own tested+committed+pushed batch (games_test / headless / minify Silk-gate green each time):

### ⚡ Round: deepen shallow games + Virtual Pet + 2 new games + mod-ban AI-approval + AI-busy UX + donate plan (games_test now 53, 0 flagged)
Design specs came from a background Workflow (design→critique per game); each game rebuilt e-ink-safe (build-once menus, per-cell/DOM diff, timers on one handle, banned-syntax clean), backward-compatible via a `_norm()` backfill on `S.games.<id>`, and each has a **seeded Daily + streak** hook + achievements.
- **Simon** (`const Simon`): mode-select menu + XP level ladder (15 tiers gate modes) + modes **Classic/Reverse/Grid-6/Grid-9/Speed Rush/Zen(3-lives)** + a seeded Daily (FNV-1a→mulberry32) + 17 achievements + best-effort Web-Audio pad tones (lazy `window._khAudioCtx`, `try/catch`, silent no-op on Silk, gated on `S.games.simon.sound`). `modeBests.classic` seeds from the legacy `best`.
- **Sokoban** (`const Sokoban`): **23 hand-verified-solvable levels** (a BFS solver in `/tmp/sokoban_verify.cjs` validated each + computed `PARS`) in 4 star-gated packs; move-par 0-3★ mastery + best-moves record; a **Level Select hub** with Continue; a static **SOLVED card** (replaced the silent auto-advance); **undo+redo** (Z/Y); ~11 achievements; a deterministic Daily. `_khSubmitScore('sokoban',solvedCount*3+totalStars)`.
- **Reversi** (`const Reversi`): **one negamax+alpha-beta search reused as 4 AI tiers**, hard-bounded by a `_nodeBudget=42000` node cap so it NEVER freezes old Silk (falls back to the heuristic eval past the cap; exact endgame only when few empties); a **12-opponent named Ladder** gating tiers, 0-3★/opponent, board themes unlocked by stars, seeded Daily (noise OFF = deterministic), 12 achievements, **last-move highlight + disc-ratio bar + Undo + limited Hint**. Free-play difficulty picker.
- **Maze** (`const Maze`): mode hub **Campaign(30 seeded levels, 5 worlds)/Daily/Endless**; **seeded gen** (mulberry32) so Campaign is fixed + Daily deterministic; **fog** (draw only cells within radius) + **locked-key** modifiers in later worlds; **BFS shortest-path → 0-3★ medals**; a **coin economy + Shop** (4 player-token skins); 12 achievements. Exposes a read-only `_dbg()` for the test harness. Key drawn as letter `K`, locked exit as a ring (no tofu).
- **Perfect Circle** (`const PerfectCircle`): **RE-ADDED** (PR #53 had deleted it — recovered the centroid/radius maths from git `0ceb158`) then deepened: mode hub **Free/Daily/Tiny/Giant/Speed(timed)**; a **4-dimension score** — roundness + closure + **full-loop COVERAGE anti-cheese** (36 angular bins; a straight line scores ~0) + size-match; 0-3★+PERFECT; seeded Daily+streak; an **8-tier rank ladder** (Wobbly Line→Giotto) + medals + a Stats screen. Re-wired ALL 7 points (launch case, exitImmersive stop-list, GAME_MAP, GAME_CATEGORY, GAME_HELP, grid card, games_test).
- **Virtual Pet** (`BUILDERS.pet`): made purposeful — a **Bond level** master track (BOND_MAX 10, gates unlocks) earned by varied care; a **branching evolution** (Adult+Bond5 → Guardian/Joyful/Radiant/Royal by care emphasis, drawn + titled); a **Shop** (instant Deluxe/Treat foods, permanent Play-boosting Toys, cosmetic Hats drawn on the sprite); **daily quests** (seeded 3/day) + a **care streak**; a **Treat Hunt** tap mini-game; 14 trophies. Sub-panel router (`panel` = main/shop/quests/trophies/game). `migrate()` backfills every new field onto legacy pets (data-safe).
- **NEW game — Lucky Roller** (`const LuckyRoller`, id `roller`, jakedajake 3-vote request): tap ROLL → 7 rarity tiers (Common→**COSMIC**, weighted, luck-skewed via `weight*pow(1+luck*0.03, tierIndex)`); backpack → **Sell (per-tier / Sell All)** for coins; **upgrades** Luck / Multi-roll / Value / **Auto-roll** (a `setInterval`, cleared in stop()); a **Collection Log** of named items. `S.games.roller`.
- **NEW game — Infinite Craft** (`const InfiniteCraft`, id `craft`, fireraider request): combine 2 things → a new one. **~110 offline seed combos** (instant, no network) + **AI (`khiCall`) for novel pairs, cached forever** in `S.games.craft.combos`; graceful fallback when the AI is busy/offline. `sanitize()` strips emoji/punct + title-cases the AI result (Silk-safe). Base = Water/Fire/Wind/Earth.
- **SECURITY — moderator bans now require AI approval** (user: "each ban has to be approved by ai so mr.incredible guy can't suddenly ban everyone"). **Client** (`_khBanUsername(name,reason)` + new `_khModBanReview`): a non-admin mod's ban is gated behind `khiCall` (approves only clearly-serious rule-breaking; DENIES petty/vague), **fails CLOSED** (no AI → no ban); admins keep direct bans. The `_khModerateUser` sheet shows "Request ban (AI-reviewed)" for mods. **Worker** (`api-worker.js` `kh_ban_username`): a non-admin mod token no longer writes `kh_banned_usernames` directly — it **queues a `[REPORT] [USERNAME] … Username:"<name>"` row** that the existing auto-moderation cron (server-side Gemini, re-derives its own verdict) or an admin actions (format matches BOTH the cron regex and the admin-panel parser). ⚠ needs api-worker REDEPLOY for the server half; the client gate is effective immediately.
- **AI-busy UX** (twix2 "always says public ai busy im new"): a keyless guest whose shared AI is overloaded no longer dead-ends on a `(…)` note — `sendChat` now degrades to the **offline AI** for ANY shared-proxy status note (not just config/reach), appending a friendly "the free AI is busy, I answered from built-in knowledge; add a free Gemini key for unlimited use" line. Overloaded strings reworded newbie-friendly.
- **`DONATE_PLAN.md`** (user "donate just as a plan"): planning doc for an optional outbound tip-jar (Ko-fi/BMC/GitHub Sponsors via `_khOpenExt`) in Settings→About — no in-app checkout, unlocks nothing. NOT shipped.
- Every new/changed game wired at ALL points (`_doLaunch` case + exitImmersive stop-list + `GAME_MAP` + `GAME_CATEGORY` + `GAME_HELP` + a `gc()` grid card + `tools/games_test.cjs` id). Tests in /tmp: `simon_test`, `sokoban_test2`, `reversi_test`/`reversi_perf`, `maze_solve`, `pc_test`, `pet_test`, `roller_test`, `craft_test`, `modban_test`, `aibusy2`.

### Earlier this session (already merged, PR #121/#122):
- **Supabase FULLY REMOVED from the repo** (user: "no mentions left, we migrated to Cloudflare long ago"). Deleted the dead in-app migration tool + admin UI, both Supabase realtime-WebSocket branches, the `SUPABASE_URL`/`SUPABASE_ANON_KEY` globals + runtime override, a ~558-line legacy Postgres-schema comment; dropped the apikey/Authorization headers the D1 Worker never reads (`_sbHeaders`/`_sbCount` now send only Content-Type). Reworded every comment/UI string + the two AI-patch denylists. Deleted `schema.sql` + `supabase-lockdown.sql` (legacy, git-recoverable). Workers/docs cleaned too. `grep -i supabase` over shipped files = 0 (CLAUDE.md keeps historical mentions). The live D1 path is unchanged.
- **EMAIL PASSWORD RESET (new)** — replaces "remember a 64-char string". Backend (`email-worker.js` `/reset/request` + `/reset/verify`, `api-worker.js` `kh_reset` table + a dedicated access gate, `schema-d1.sql`): 6-digit code, salted-hash storage (`SHA-256(u+':'+code)`), 10-min expiry, single-use, ≤5 attempts, 1/60s + 5/hr pacing, DAILY_SEND_CAP, `timingSafeEqual`, no account-existence leak. `kh_reset` is server-only, gated by `X-KH-Reset-Secret === env.RESET_SECRET` OR admin (open read would let `u`-holders brute-force the 6-digit; open write = reset bypass). Client (`index.html`): `_khForgotPasswordDialog` now leads with an email path (`u = hex SHA-256 of the canonical username`; `_khResetPost` hits `_mailGatewayUrl()+/reset/*`) → verify → restore with the recovery code (E2E: still needed to decrypt data) OR start fresh; falls back to the recovery-code-only screen when no mail gateway (never dead-ends). New `S.recoveryEmail` + a Settings "Recovery email" field + "Email me my recovery code" button (`_khEmailMyRecoveryCode`, puts the code in the inbox), a one-time existing-user popup (`_khMaybeAskRecoveryEmail`, boot +3.4s, non-stacking), and an onboarding 'name'-step field. `_khRecoverWithCode`/auth internals untouched. ⚠ **DEPLOY GATE:** set `RESET_SECRET` (same value) on BOTH api-worker + email-worker, plus `RESEND_API_KEY` (+ optional `RESET_FROM`, default `noreply@kindlehub.pro`); until then the client auto-falls-back to the recovery-code flow.
- **KindleOS lock screen fixes**: the desktop icon grid no longer builds UNDER the lock (deferred to first unlock via `_kdBuildGridOnce`, called in `unlock()`) — fixes the glitchy/slow "glass" load; and a swipe-up unlock no longer ALSO fires the corner quick-note/screenshot (capture lock state at touchstart via `_cnrLockedAtStart`, not touchend).
- **Games page reorganised** (ReKindle-style): `GAME_CATEGORY`/`GAME_CAT_ORDER`/`GAME_CAT_LABEL` + `gameCategory(id)` bucket all games into searchable category shelves (Arcade/Puzzle/Word/Board & Cards/Strategy & Sim/Casual/More) with a debounced search box + jump-chips in `BUILDERS.games`. No game removed; cards moved not rebuilt. **New games MUST get a `GAME_CATEGORY` entry** or they fall to "More".
- **Stocks "always Stale" FIXED**: the freshness label was built once in `renderMain()` and never refreshed, so live fetches updated the grid but not the label. Now `freshDiv` + `updateFreshLabel()` (called from `renderGrid`/`fetchAll`/`fetchOne`), keyed off the NEWEST successful fetch (one stubborn ticker can't mark all stale).
- **Zoo/Animals upgrade**: `BUILDERS.animals` 31→78 animals (new `cons` conservation field + badge, backfilled), a Compare-two-animals mode, and a 5-question DB-generated quiz.
- **App Store publisher AGE RATINGS**: `ageRating` (Everyone/9+/12+/17+, default Everyone) on `S.publishedApps`, chosen in Publish, badged on every community card + Community Games, + a `S.showMatureApps` (default off) 17+ filter. Helpers `KH_AGE_RATINGS`/`_khNormAge`/`_khAgeBadge`. Rating lives in the synced app object only (no `kh_store_apps` DB column — would need a worker change). **Security re-verified**: published apps STILL cannot reach external servers — CSP `default-src/connect-src 'none'` (`_khAppCsp`, unchanged) + sandbox without `allow-same-origin`/`allow-popups` (`_khOpenPublishedApp`, unchanged); a headless test confirmed a sandboxed app's `fetch` is BLOCKED.
- **schema-d1.sql ↔ Worker SCHEMA_DDL synced** (added `kh_state_parts`/`kh_daily`/`kh_config` + `kh_announcements.comments` + `kh_presence.avatar`).
- New planning docs: **`PRICING_PLAN.md`** (1600 future subscription tiers) + **`ACCOUNT_V2_PLAN.md`** (split the DB lookup key from the AES key via salt+PBKDF2/HKDF; dual-read→migrate-on-login→sunset — planning only).
- Reddit post for r/kindlejailbreak was delivered in CHAT (not committed).
- **Stars** (`BUILDERS.starmap`): the Stellarium-like core (gnomonic straight-horizon projection, ~2061-star + 88-constellation-figure catalog from packed `_SD_*` strings, named constellation labels, the search FAB over stars/DSOs/sats/planets) shipped earlier this session-arc and is committed. Remaining polish (live planets/moon-phase, tap-to-identify panels) is optional follow-up.
- **Standalone-game-page fallback (#92) — DEFERRED with rationale**: on a single-file app a "new tab" still parses the whole ~3 MB bundle (no parse-time win; only a fresh heap), and a real solo-boot path would touch the shared boot sequence for 350 users. Left for a user architecture call rather than risk boot. Delivered instead: the games *reorg* (committed).
- **STILL PENDING next session (a subagent was cut off by the Max-5x session limit; its partial, untested index.html edit was REVERTED so the repo stayed clean):** (1) **more new games + Crossy Road v2 (#92/#93)** — NOT done. (2) **Settings reorg + visual polish (#85)** — NOT done. (3) optional Stars polish (planets/moon/tap-identify). Every other item in this session's list above IS committed + pushed and verified (minify Silk-gate + headless).
